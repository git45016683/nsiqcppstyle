"""
Indent the each struct item in the struct block.
...
def RunRule(lexer, typeName, typeFullName, decl, contextStack, typeContext) :
    if not decl and typeName == "STRUCT" and typeContext != None:
...
#            if typeContext != t.contextStack.Peek() : continue
            if GetRealColumn(t) <= (column + 1):
                nsiqcppstyle_reporter.Error(t, __name__, "Struct block should be indented. But the token(%s) seems to be unindented" % t.value);
